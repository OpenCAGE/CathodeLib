...
namespace CATHODE.Misc
{
    ...
    public class CathodeStringDB
    {
        private string filepath;
        public List<StringDBEntry> dbEntries = new List<StringDBEntry>();

        /* Load the file */
        public CathodeStringDB(string path)
        {
            filepath = path;
        ...
        public void Save()
        {
            ...
            {
                Utilities.Write<cGUID>(writer, dbEntries[i].ID);
                writer.Write(i);
            }
            for (int i = 0; i < dbEntries.Count; i++)
            {
        ...
        {
            public StringDBEntry(cGUID _id, string _str)
            {
                ID = _id;
                content = _str;
            }
